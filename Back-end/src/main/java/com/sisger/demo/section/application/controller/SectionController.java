...
public class SectionController implements SectionControllerInterface {
    ...
    @Override
    public ResponseEntity<ResponseSectionDTO> findByID(String token, String id) {
        log.info("[inicia] SectionController - findByID");
        var user = tokenService.getUserByToken(token);
        AuthorityChecker.requireManagerAuthority(user);
        log.info("[fim] SectionController - findByID");
        return ResponseEntity.ok().body(sectionService.findBySectionId(id));
    }
    ...
    @Override
    public void deleteSection(String token, RequestDeleteSectionDTO requestDeleteSectionDTO) {
