...
public class SectionService implements SectionServiceInterface{
    ...
    @Override
    public ResponseSectionDTO findBySectionId(String id) {
        log.info("[inicia] SectionService - findBySectionId");
        Section section = this.findById(id);

        log.info("[fim] SectionService - findBySectionId");

        return ResponseSectionDTO.builder()
                .id(section.getId())
                .name(section.getName())
                .company(companyService.buildResponseCompanyChildDTOFromCompany(section.getCompany()))
                .build();
    }
