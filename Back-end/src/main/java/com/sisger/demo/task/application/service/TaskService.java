...
public class TaskService implements TaskServiceInterface{
    ...
    @Override
    public ResponseTaskDTO findByTaskIdResponse(String taskId, User manager) {
        log.info("[inicia] TaskService - findByTaskIdResponse");

        var task = this.findById(taskId).orElseThrow(() ->  new NotFoundException("Task not found"));
        verifyCompanyMatch(task.getSection().getCompany(), manager.getCompany());
        log.info("[fim] TaskService - findByTaskIdResponse");
        return  ResponseTaskDTO.builder()
                ...
                .section(sectionService.findBySectionId(task.getSection().getId()))
                .build();
    }
    ...
    @Override
    public void delete(String id, User manager, String password) {
        log.info("[inicia] TaskService - delete");

        Task task = this.findById(id).orElseThrow(() -> new NotFoundException("Task not found"));

        PasswordHandler.validatePassword(password, manager.getPassword(), passwordEncoder);
